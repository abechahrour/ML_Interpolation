...
######################################################
    ...
    if func == "D1" or func == "D3" or func == "D6" or func == "D9":
        (f, f_interp, integral_f,
        x_train, y_train, x_test, y_test) = cm.get_functions(func, ndims, alpha)
        scaler = cm.get_scaler(set_scaler, low = low, high = high, load = load, NAME = NAME)
        y_transf = scaler.fit_transform(y_train.reshape(-1, 1))
        tri = Delaunay(x_train)  # Compute the triangulation
        interpolator = LinearNDInterpolator(tri, y_transf)
        nearest = NearestNDInterpolator(x_train, y_transf)
        y_near = np.squeeze(nearest(x_test))
        start = time.time()
        y_pred = np.array(interpolator(x_test)).squeeze()
        end = time.time()
        pred_time = end - start
        y_pred[np.isnan(y_pred)] = y_near[np.isnan(y_pred)]
    ...
    cm.plot_nearest(y_test, y_pred, measures, 'linear', NAME)
